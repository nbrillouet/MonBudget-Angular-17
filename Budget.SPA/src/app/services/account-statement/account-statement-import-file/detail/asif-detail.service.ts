...
export class AsifDetailService extends CorpDataForm<AsifForDetail,AsifForDetailFormBuilderOptions>
{
    ...
    private changeOperationTypeList(operationTypeFamily: SelectCode): void {
        this._store.dispatch(new AsifDetailFilterChangeOt({idAccount: this.form.getValue(x=>x.account)?.id,idMovement: this.form.getValue(x=>x.idMovement), ot: this.form.getValue(x=>x.operationType), otf: operationTypeFamily }));
    }
