...
export class ProfileComponent implements OnInit, OnDestroy
{
    ...
    @ViewChild('tagsPanel') private _tagsPanel: TemplateRef<any>;@ViewChild('tagsPanelOrigin') private _tagsPanelOrigin: ElementRef;

    property: UserForDetail;
    x: any;
    toogleAvatar: boolean = false;
    y: any;
    enumHeaderTab: typeof EnumHeaderTab = EnumHeaderTab;
    myHeaderTab: EnumHeaderTab = EnumHeaderTab.info;

    // uploader: FileUploader = new FileUploader({
    //     url: `${environment.apiUrl}users/${this._authService.user.id}/avatar`,
    //     authToken: `Bearer ${this._authService.user.token}`,
    //     isHTML5: true,
    ...
    {
        if(this.idUser)
            { this._profileService.load(this.idUser); }
        else
            { this._profileService.load(this._authService.user.id ); }
        ...
        //             if(!this._helperService.areEquals(x,this.y)) {
        //                 this.y=this._helperService.getDeepClone(x);
        ...
        // });
        // this.subscribeDetail = this.detail$.subscribe((x) => {
        //     if (x?.loader['datas']?.loaded) {
        //
        //     }
        // });
    ...
    toogleAvatarEvent(value: boolean): void {
        this.toogleAvatar = value;
     ...
     * Close the drawer
     */
    closeDrawer(): any //Promise<MatDrawerToggleResult>
    {
    ...
    uploadAvatar(fileList: FileList): void
    {
        ...
        // Upload the avatar
        this._profileService.changeAvatar(Number(this._profileService.form.getValue(x=>x.id)), formData);
    }
